<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on("click", ".open-edtModal", function () {
            var id = $(this).data('id');
            $(".modal-body #ed_id").val( $(this).data('id') );
            console.log(id);
            if(id > 0){
                $.ajax({
                    url: '{{ route('personal.getErContact') }}',
                    type: 'get',
                    data: {id:id},
                    dataType: 'json',
                    success:function(data){
                        $('#ed_name').val(data['ed_name']);
                        $('#ed_relationship').val(data['ed_relationship']);
                        $('#ed_date_of_birth').val(data['ed_date_of_birth']);
...
<form id="contactForm" action="{{ route('setcontact') }}" method="POST">
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
    <input type="hidden" name="emp_number" value="{{ $emp->emp_number }}" />
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
        <div class="form-group">
            <label for="emp_street1">ID Address</label>
            <textarea rows="5" class="form-control" type="text" name="emp_street1" id="emp_street1" readonly="readonly">{{ $emp->emp_street1 }}</textarea>
        </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
        <div class="form-group">
            <label for="emp_street2">Residence Address</label>
            <textarea rows="5" class="form-control" type="text" name="emp_street2" id="emp_street2" readonly="readonly">{{ $emp->emp_street2 }}</textarea>
        </div>
    </div>
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
        <div class="form-group">
